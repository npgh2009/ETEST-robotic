...
void setup (){
    ...
    //---------------Encoder------------------------------------------
    //Note:  timer 1 pin 9,10, timer 0 pin 5,6 (default 976Hz) ; timer 2 pin 3,11
...
void updateMPU(){
	...
	// khi goi ham Calib_ax o Step_0 se co tac dung thay doi calib_ax ở các step sau đó --------------
 	if ((stepp > 0) && (countt > 0 )){
 		calib_ax = sum_calib_ax / countt;  
 	}
   
 	if ((stepp > 0) && (countt_gz > 0 )){
 		calib_gz = sum_calib_gz / countt_gz;     
    }
    ...
    gz_value = gz_kalman / 32767*250  - calib_gz ;
 	...
 	if (stepp > 0){
 		anglez += (now-pre)*gz_value/1000; // Calculate angle
 	}
...
class PID{
  	private:
  		...
  		float past = 0;
  	public:
  		float calculate(float ref, float feedback, float Kp, float Ki){
            now = millis();
            ...
            sum = sum + e*T; //Calculate integral
            pre_e = e;
            past = now;
            return u;
            }
...
void PID_position(float ref_x, float ref_y){
	...
	v_xd = positionx.calculate(ref_x, feedback_x, Kp, Ki);
	v_yd = positiony.calculate(ref_y, feedback_y, Kp, Ki);
...
void PID_gocquay(float ref,float v_d){
	...
	float Ki = 10;
  	...
  	float w_d = goc.calculate(ref, feedback_anpha, Kp, Ki);
...
void PID_MotorSpeed(float ref1, float ref2){
	...
	feedback2= Read_AngularVelocity_Right();
  	...
  	pwmOut1(motor1.calculate(ref1, feedback1, Kp, Ki));  //output1 Pwm
  	pwmOut2(motor2.calculate(ref2, feedback2, Kp, Ki));   //output2 Pwm
...
void PID_angleOfRot(float refff1, float refff2){
	...
	int temp1 = AOR_Left.calculate(refff1, temp3, Kp, Ki); // Use PID to calculate what signal should we output
	int temp2 = AOR_Right.calculate(refff2, temp3, Kp, Ki); // temp1 sẽ bằng temp2
...
void PID_angleOfCar(float ref){
	...
	int temp = AOC.calculate(ref, feedback_angle, Kp, Ki);
	spin = temp;
